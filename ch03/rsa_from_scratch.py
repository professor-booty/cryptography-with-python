...
def generate_prime_candidate(length):
    """Generate a random off integer of a specified bit length.
...
def encrypt(pk, plaintext):
    ...
    plaintext: A strign representing the message to be encrypted
    ...
    # using more efficient modular exponentiation
...
def decrypt(pk, ciphertext):
    ...
    Args:
    pk: A tuple containing the private key and modulus (e, n).
    ciphertext: A list of integers representing the encypyted message.

    Returns:
    A string representing the decrypted message"""
    key, n = pk
    # Decrypted each number in the ciphertext and convert back to characters
